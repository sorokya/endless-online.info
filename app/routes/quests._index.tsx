...
import type { Route } from './+types/quests._index';

import { useDebounce } from '@uidotdev/usehooks';
import { Form, Link, data } from 'react-router';
import { getQuestList } from '~/.server/quests';
import { CONFIG } from '~/config';

export function links() {
  return [{ rel: 'canonical', href: 'https://endless-online.info/quests' }];
}
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const quests = await getQuestList(search);
  return data({
    quests,
    search,
  });
...
export default function Quests({ loaderData }: Route.ComponentProps) {
  const { quests, search } = loaderData;
  ...
  const pageCount = useMemo(
    () => Math.ceil(quests.count / CONFIG.PAGE_SIZE),
    [quests],
  );
        ...
        <>
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            {quests.records.map((quest) => (
              ...
              >
                <img
                  src={`https://eor-api.exile-studios.com/api/npcs/${quest.npc_id}/graphic`}
                  alt={quest.name}
                  className="h-16 w-full object-contain"
                />
                <div className="mt-2 text-center font-bold">{quest.name}</div>
              </Link>
            ))}
          </div>
          {pageCount > 1 && (
